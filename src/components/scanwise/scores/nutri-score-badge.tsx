...
import { NUTRI_SCORE_CONFIG } from '@/lib/constants';
...
const sizeConfig = {
  ...
  lg: {
    ...
    label: 'text-sm',
    glow: 'shadow-[0_0_28px_rgba(34,197,94,0.5)]',
  },
} as const;

const gradeColors: Record<NutriScoreGrade, { bg: string; glow: string; text: string; ring: string }> = {
  A: {
...
export function NutriScoreBadge({
  ...
  const config = NUTRI_SCORE_CONFIG[grade];
  ...
  return (
    ...
    >
      <motion.div
        className={cn(
          'relative flex items-center justify-center rounded-2xl ring-2',
          sizeStyles.container,
          colors.bg,
          colors.text,
          colors.ring,
          isHighScore && sizeStyles.glow,
      ...
      >
        <span className={cn(sizeStyles.text, 'font-display tracking-tight')}>
          {grade}
        </span>
